...
\begin{itemize}
    ...
    \item $\text{spatial\_random\_effect}_{i}$ accounts for spatial autocorrelation. To account for spatial autocorrelation, we incorporate a Besag-York-Molli\'{e} (BYM) specification \citep{besag1991bayesian}. This approach decomposes the $\text{spatial\_random\_effect}_{i}$ into a mixture of two distinct components to distinguish between regional trends and localized anomalies:
    \begin{itemize}
...
For the $\text{covariates\_effect}_{i,t}$ we introduce the time dimension $t$, and we use the following covariates:
